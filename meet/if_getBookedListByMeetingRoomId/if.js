...
function run(Param, Robot, Request, Response, IF) {
    ...
    if(arg.startTime!=undefined&&arg.startTime!=""){
       var startTime = parseInt(arg.startTime);
       var daybtime =util.getMaxtimeformdata(startTime);
       var dayetime =util.getmintimeformdata(startTime);
       query ={
        state : {
      ...
      var daystime = util.getMaxtimeformdata(new Date().getTime());
      query ={
        state : {
            $in : [1, 2, 5, 7]//1为审核中，2预定成功5，会议室结束 ，7 会议中 
        },
    ...
    console.log("条件:"+JSON.stringify(query));
    meetBookModel.find(query,{"userId":1,"userName":1,"state":1,"startTime":1,"endTime":1}).sort({
        "startTime" : 1
