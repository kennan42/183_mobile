...
/**
 *
 * 同步PA9112  技术职务级别  增量或全量 更新，PRD分别在145，150上启动同步程序
 *  如果145已经同步成功，则150上不再进行数据同步
 * 1 判断今天是否已经进行了数据同步（syncPA9112:2008-08-08）
...
function run(Param, Robot, Request, Response, IF) { 
    ...
    redisCli.get("syncPA9112", function(err, data) {
    redisCli.quit();
    if (data == null) {//全量更新
    udpateAll = true;
    cb(null, "");
    ...
    console.log("-------async  2");
    if (!err && data1.ES_PUBLIC.TYPE == 0 && data1.ET_PA9112.item != null) {
    var rs = data1.ET_PA9112.item;
    ...
    $set:{
                        ...
                        "ZZ_JSZWLB" : user.ZZ_JSZWLB, //技术职务类型
                        "ZZ_JSZWJB":user.ZZ_JSZWJB,  //技术职务级别
                        "ZZ_JSZWJBT":user.ZZ_JSZWJBT,//描述  1级 2级3级
...
function(cb) {
    ...
    redisCli.on("ready", function() {
        redisCli.select(contactUtil.contactConst.redisDB, function() {
            redisCli.set("syncPA9112", currentDate, function(err) {
                redisCli.quit();
                cb(null, "");
            });
        });
    ...
    Response.end("sync PA9112 over");
    console.log("sync PA9112 over");
});
}
