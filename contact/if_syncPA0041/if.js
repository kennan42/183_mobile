...
/**
 *
 * 同步PA0041 入司时间相关增量或全量 更新，PRD分别在145，150上启动同步程序
...
function run(Param, Robot, Request, Response, IF) { 
    ...
    var option = {
    "wsdl" : path.join(__dirname.replace(IF.name, ""), global.wsdl, "zhrtxws08.xml"),
    "func" : "ZHRTXWS08.ZHRTXWS08.ZHRTXWS08",
    "Params" : reqParam,
    "agent":false
    };
    MEAP.SOAP.Runner(option, function(err, res, data1) {
    console.log("-------async  2");
    if (!err && data1.ES_PUBLIC.TYPE == 0 && data1.ET_PA0041.item != null) {
    var rs = data1.ET_PA0041.item;
    ...
    "PERNR" : userId
                        ...
                        "ZZ_RZSJ" : user.DARDT, //入职时间
                        "DATAR":user.DATAR,  //入职时间日期类型
...
function(cb) {
    ...
    redisCli.on("ready", function() {
        redisCli.select(contactUtil.contactConst.redisDB, function() {
            redisCli.set("syncPA0041", currentDate, function(err) {
                redisCli.quit();
                cb(null, "");
            });
        });
    ...
    Response.end("sync PA0041 over");
    console.log("sync PA0041 over");
});
}
