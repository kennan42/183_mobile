...
/** 
 *  
 * 同步PA0000 员工状态增量或全量 更新，PRD分别在145，150上启动同步程序
...
补充说明：在IM_ADDDATA传值，传入日期IM_KEYDATE不传值时，默认取得当前日期前2天至今的增量数据，建议传入日期为当前日期前2天
    ...
    var users = [];
    //调用webservice返回的员工信息
    var currentDate = util.getDateStrFromTimes(new Date().getTime(), false);   //2016-03-01，这里更新 只能具体到某一天
    
    var date = currentDate.replace(/-/g , "");    //20160301
    ...
    function(cb) {
        ...
        redisCli.on("ready", function() {
            redisCli.select(contactUtil.contactConst.redisDB, function() {
                redisCli.get("syncPA0000", function(err, data) {
                    redisCli.quit();
                    if (data == null) {//全量更新
                        udpateAll = true;
                        cb(null, "");
    ...
    //调用webservice接口同步用户信息
        ...
        function(cb) {
            ...
            var option = {
                "wsdl" : path.join(__dirname.replace(IF.name, ""), global.wsdl, "zhrtxws05.xml"),
                "func" : "ZHRTXWS05.ZHRTXWS05.ZHRTXWS05",
                "Params" : reqParam,
                "agent":false
            };
            MEAP.SOAP.Runner(option, function(err, res, data1) {
                console.log("-------async  2");
                if (!err && data1.ES_PUBLIC.TYPE == 0 && data1.ET_PA0000.item != null) {
                    var rs = data1.ET_PA0000.item;
    ...
    function(cb) {
        ...
        redisCli.on("ready", function() {
            redisCli.select(contactUtil.contactConst.redisDB, function() {
                redisCli.set("syncPA0000", currentDate, function(err) {
                    redisCli.quit();
                    cb(null, "");
                });
            });
        ...
        Response.end("sync PA0000 over");
        console.log("sync PA0000 over");
    });
}
