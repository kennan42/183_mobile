...
/**
 *
 * 同步PA9120 省描述和办公地描述 相关增量或全量 更新，PRD分别在145，150上启动同步程序
...
function run(Param, Robot, Request, Response, IF) {
    ...
    function(cb) {
        ...
        redisCli.on("ready", function() {
            redisCli.select(contactUtil.contactConst.redisDB, function() {
                redisCli.get("syncPA9120", function(err, data) {
                    redisCli.quit();
    ...
    function(cb) {
        ...
        MEAP.SOAP.Runner(option, function(err, res, data1) {
            console.log("-------async  2");
            if (!err && data1.ES_PUBLIC.TYPE == 0 && data1.ET_PA9120.item != null) {
                var rs = data1.ET_PA9120.item;
                ...
                setTimeout(function() {
                    ...
                    userModel.update({
                        ...
                        $set : {
                            ...
                            "ZZ_CZD" : user.ZZ_CZD,
                            "ZZ_CZDA" : user.ZZ_CZDA, //省
                            "ZZ_CZDA_T" : user.ZZHR_BASE1T, //描述
                            "ZZ_CZDB" : user.ZZ_CZDB,
                            "ZZ_CZDB_T" : user.ZZHR_BASE2T,
                            "ZZ_CZDC" : user.ZZ_CZDC,
                            "ZZ_CZDC_T" : user.ZZHR_BASE3T,
                            "ZZ_LDBG" : user.ZZ_LDBG,
                            "ZZ_CZD1" : user.ZZ_CZD1,
                            "ZZ_CZDA1" : user.ZZ_CZDA1,
                            "ZZ_CZDA1_T" : user.ZZHR_BASE1T1,
                            "ZZ_CZDB1" : user.ZZ_CZDB1,
                            "ZZ_CZDB1_T" : user.ZZHR_BASE2T1,
                            "ZZ_CZDC1" : user.ZZ_CZDC1,
                            "ZZ_CZDC1_T" : user.ZZHR_BASE3T1,
    ...
    function(cb) {
        ...
        redisCli.on("ready", function() {
            redisCli.select(contactUtil.contactConst.redisDB, function() {
                redisCli.set("syncPA9120", currentDate, function(err) {
                    redisCli.quit();
                    cb(null, "");
                });
            });
        ...
        Response.end("sync PA9120 over");
        console.log("sync PA9120 over");
    });
}
