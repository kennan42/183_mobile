...
appcan.define("slider", function(a, b, c) {
    //自定义，拼接div
    var d = '<div class="slider-item ub-fh ub-fv ub-img1" style="background-image:url(<%=data.img%>)"><%=data.div%></div>',
    ...
    g.prototype = {
        ...
        _moveTo : function(a, b) {
            ...
            //alert(c.focus.parents("#slider").next().html());
            c.focus.parents("#slider").next().find("li").removeClass("on");
            c.focus.parents("#slider").next().find("li").eq(c.option.index).addClass("on");
            //增加回调
            c.emit("slideTransitionEnd", c, a, b);
        ...
        set : function(b) {
            ...
            return c.option.hasLabel && (c.label = c.label || a('<div class=""></div>'), c.ele.append(c.label)), c.focus = c.focus || a('<div class=""></div>'), c.focus.css("width", f), c.focus.css("-webkit-transform", "translateX(" + c.option.index * f + "px)"), c.ele.append(c.focus), c._moveTo(c.option.index, !1), c.ele.on("swipeMoveLeft", function(a) {
