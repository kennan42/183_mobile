...
(function(){
    ...
    function popoverBounceSetting(popName,enableBounce,popBgcolor){
        ...
        top.hiddenBottomPopoverTimer = setTimeout(function(){
            if(top.hiddenBottomPopoverTimer){clearTimeout(top.hiddenBottomPopoverTimer);}
            var popList = Zepto("div[id^='pop_']");
            Zepto.each(popList,function(i,v){
                if(top.uexWindow._iscroll.popName && popList[i].id && popList[i].id !=top.uexWindow._iscroll.popName){
                    //Zepto(popList[i]).css("display","none !important");
                }
            });
        },2000);
    }
    ...
    window.uexWindow = {
        ...
        openPopover: function(inWindName, inDataType, inData, data, x, y, w, h, fontSize, type, bottomMargin, extraInfo) {
            ...
            popContainer = Zepto('<div id="pop_' + inWindName + '" class="batou_zhy" ></div>');
        ...
        closePopover: function(inPopName) {
            ...
            Zepto = top.Zepto || Zepto;
            var popList = Zepto("div[id^='pop_']");
            Zepto.each(popList,function(i){
                if(popList[i].id !="pop_" + inPopName){
                    Zepto(popList[i]).css("display","block !important");
                }
            });
        ...
        alert: function(inTitle, inMessage, inButtonLable) {
            ...
            var alertHtml = '<div class="loader_alert up ub ub-ver" style="background-color:rgba(0,0,0,.5);display:none !important;"><div class="ub-f1 tx-l t-bla zhy_top" ><div class="ub ub-ver  zhy_win"><div class="uuin_1 uc-t1" ><div class="ub ub-ver zhy_bg_1 t-wh uc-a "><div class="zhy_uinn uc-t1 tx-c ubb u_border"><div class="uinn loader_alert_1">提示</div><div class="uinn loader_alert_2"> 一旦提交，无法撤销</div></div><div class="tx_color " onclick="uexWindow.closeAlert()"><div class="ub tx-c"><div class=" ub-f1 zhy_pading loader_alert_3">确定</div></div></div></div></div></div></div></div>';
        ...
        actionSheet: function(inTitle, inCancel, inButtonLables) {
            ...
            var plugincss = jsBasePath + "js/resource/appcan.plugin.css";
            loadjscssfile(plugincss,"css");
            var actionSheetHtml = '<div class="loader_actionsheet up ub ub-ver" style="background-color:rgba(0,0,0,.5);display:none !important;"><div class="ub-f1 tx-l t-bla"><div class="ub ub-ver  zhy_winss"><div class=" uc-t1"><div class="ub ub-ver  t-wh " style="background:white;color:black"><div class=" uc-t1 tx-c"><div class="uinn loader_actionsheet_1">提示</div><div class="uinn loader_actionsheet_2"></div></div><div class=" "><div class="ub tx-c" onclick="uexWindow._cbActionSheet()"><div class=" ub-f1 zhy_pading loader_actionsheet_3">取消</div></div></div></div></div></div></div></div>';
        ...
        bringPopoverToFront: function(inName) {
            ...
            var popList = Zepto("div[id^='pop_']");
            var popTopIframe = Zepto("#iframe_pop_"+inName);
            
            //不存在置顶的popover则结束
            if(!popTopIframe || (popTopIframe && !popTopIframe.length)){
                return;
            }
            
            Zepto("#pop_" + inName).css({
                'zIndex':ZIndex,
                'display':'block !important'
            });
            
            //隐藏未置顶的popover,解决打开多个popover时部分手机滑动时卡顿的bug,如小米4
            Zepto.each(popList,function(i,v){
                var contentWrapper =Zepto(popList[i].querySelector("iframe").contentDocument.querySelector("#selfWrapper")); 
                if(popList[i].id && popList[i].id !="pop_"+inName){
                    
                 //   Zepto(popList[i]).css("display","none !important");
                    
                    //主要针对ios
                    contentWrapper.css("overflow","hidden");
                }else if(popList[i].id && popList[i].id == "pop_"+inName){
