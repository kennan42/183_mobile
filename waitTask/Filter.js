 ...
 * 
 *1.费用报销停机配置
 * 如果在代办中 ，代办列表的数据走hana。所以需要拦截具体固定的接口
 *waitDetail（未办）的时候需要通过“接口（waitDetail）”和“入参（BUSSCATG : "F010"）”来判断是否为费用报销是否停机；
 * complistDetail_cost（已办），需要通过接口来判断
 */
function run(Param, Robot, Request, Response, IF, cb) {
    ...
    var path = Param.path;//接口方法
    var type = Param.type;//接口
    var arg = JSON.parse(Param.body.toString());//接口入参
    console.log("alalalalaalalal::::::",arg);
    console.log("alalalalaalalal::::::",arg.BUSSCATG);
                ...
                if (res.length > 0) {
                    ...
                    if(path=="complistDetail_cost"){
                        Response.end(JSON.stringify({
                                 ...
                                 return;
                    }else if(path=="complistDetail_cost"){
                        if(arg.BUSSCATG=="F010"){
                           Response.end(JSON.stringify({
                               status: -9999,
                               msg: "已停机",
                               data: res[0]
