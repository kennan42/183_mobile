...
function run(Param, Robot, Request, Response, IF)
{
    ...
    travelModel.find({"$or":[{startDate:{$gte:new Date().getTime()},company:company,userId:userId,carpoolType:1,userType:{$in:[2,4]},state:0}
                            ,{setCloseTime:{$gte:new Date().getTime()},company:company,userId:userId,carpoolType:2,userType:{$in:[2,3]},state:0}]}).populate("car")
